...
void sha256_update(sha256_ctx_t *ctx, const BYTE *data, size_t len) {
    ...
    if (index > 0) {
        ...
        memcpy(ctx->buffer + index, data, to_copy);
...
void sha256_final(sha256_ctx_t *ctx, BYTE *digest) {
    size_t index = ctx->count % SHA256_BLOCK_SIZE;
    uint64_t bit_length = ctx->count * 8;
    ...
    for (int i = 0; i < 8; i++) {
        ctx->buffer[63 - i] = (bit_length >> (i * 8)) & 0xFF;
    }
...
int sha256_file_hash(const char *filename, BYTE *digest, size_t chunk_size) {
    ...
    printf("Hashed %zu bytes from '%s' in chunks of %zu bytes\n",
