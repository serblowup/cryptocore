...
void print_usage(const char *program_name) {
    ...
    fprintf(stderr, "  Encryption with provided key:  %s --algorithm aes --mode cbc --encrypt --key 000102...0f --input plain.txt --output cipher.bin\n", program_name);
    ...
    fprintf(stderr, "  NIST tests:                   %s --input --test-nist\n", program_name);
}
...
int parse_arguments(int argc, char *argv[], config_t *config) {
    ...
    if (strlen(config->algorithm) == 0 && strlen(config->input_file) > 0) {
        if (strcmp(config->input_file, "--test-keys") != 0 &&
            strcmp(config->input_file, "--test-nist") != 0) {
            fprintf(stderr, "Error: --algorithm is required for encryption/decryption\n");
            return 0;
        }
    }

    if (strcmp(config->algorithm, "aes") != 0 && strlen(config->algorithm) > 0) {
        fprintf(stderr, "Error: Only 'aes' algorithm is supported\n");
        return 0;
    }

    if (!encrypt_flag && !decrypt_flag && strlen(config->algorithm) > 0) {
    ...
    if (strlen(config->output_file) == 0 && strlen(config->algorithm) > 0) {
